...
export function displayToFullClass(
	type: ShadowLayerType,
	displayValue: string,
): string {
	const prefix =
...
export function fullClassToDisplay(
	type: ShadowLayerType,
	fullClass: string,
): string {
	const prefix =
			...
			: type === "inset-shadow"
				...
				: type === "ring"
					...
					: type === "inset-ring"
						? "inset-ring-"
						: "text-shadow-";
	return fullClass.startsWith(prefix)
...
function extractSizeValue(sizeClass: string, prefix: string): string {
	return sizeClass.startsWith(prefix)
		? sizeClass.slice(prefix.length)
		: sizeClass;
}

function shadowSizeToCSS(
	size: string,
	color?: string,
	defaultColorCSS?: string | null,
): string {
