...
export function GradientEditor(props: GradientEditorProps) {
  ...
  const { refs, floatingStyles, context } = useFloating({
    ...
    onOpenChange: (open) => { if (!open) state.handleCloseColorPicker(); },
    strategy: 'fixed',
      ...
      <div className="flex items-start gap-2.5 mb-2">
        ...
        {isSolid ? (
          ...
          <div
            ...
            title={state.solidColorName ? `bg-${state.solidColorName}` : 'Click to pick color'}
          >
            {!state.solidColorHex && (
              <svg
              ...
              >
                <line x1="4" y1="4" x2="96" y2="96" stroke="#F5532D" strokeWidth="3" strokeLinecap="round" />
                <line x1="96" y1="4" x2="4" y2="96" stroke="#F5532D" strokeWidth="3" strokeLinecap="round" />
              </svg>
            )}
          </div>
        ) : (
      ...
      {state.colorPickerOpen && (
        <FloatingPortal>
          <div
            ...
            className="p-2 bg-bv-surface border border-bv-border rounded-md shadow-lg"
            {...getFloatingProps()}
          >
            <ColorGrid
              ...
              onRemove={state.editingTarget === 'solid' ? state.handleRemoveSolid : undefined}
              onRemoveHover={state.editingTarget === 'solid' ? state.handleHoverRemoveSolid : undefined}
            />
            {state.selectedStopIsRemovable && (
              <button
                ...
                onClick={() => {
                  if (state.selectedStopId) state.handleStopRemove(state.selectedStopId);
                }}
              >
                Remove stop
              </button>
