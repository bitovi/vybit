		...
		ws.on("message", (raw) => {
			try {
				...
				if (msg.type === "REGISTER") {
					const role = msg.role;
					if (role === "overlay" || role === "panel" || role === "design" || role === "canvas") {
						clientRoles.set(ws, role);
						console.error(`[ws] Client registered as: ${role}`);
						if (role === "panel") {
							ws.send(
								JSON.stringify({ type: "QUEUE_UPDATE", ...getQueueUpdate() }),
							);
						}
					}
					return;
				...
				if (msg.to) {
					...
					if (
						(msg.type === "COMPONENT_ARM" || msg.type === "COMPONENT_DISARM") &&
						msg.to === "overlay"
					) {
				...
				if (msg.type === "PATCH_STAGED") {
					...
					console.error(
						`[ws] Commit created: #${commit.id} (${commit.patches.length} patches)`,
					);
					broadcastPatchUpdate();
					...
					const patch = addPatch({
						...msg.patch,
						kind: msg.patch.kind ?? "component-drop",
					});
