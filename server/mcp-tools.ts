...
import type { Commit, Patch, PatchStatus } from "../shared/types.js";
...
function buildCommitInstructions(
	commit: Commit,
	...
	const componentDrops = commit.patches.filter(
		(p) => p.kind === "component-drop",
	);
...
|---|-----------|--------|-------|----------|------|
${patch.canvasComponents
	.map((c: any, i: number) => {
		const importPath = c.componentPath
			? c.componentPath.replace(/\.tsx?$/, "")
			: "—";
		const props = c.args
...
`;
			...
			if (patch.targetPatchId && patch.targetComponentName) {
				...
				const tag = patch.target?.tag ?? "element";
				const classes = patch.target?.classes
					? ` class="${patch.target.classes}"`
					: "";
...
- **Component:** \`${comp}\`
- **Element:** \`<${tag}>\`
- **Text change:** "${patch.originalText}" → "${patch.newText}"
${context ? `- **Context HTML:**\n\`\`\`html\n${context}\n\`\`\`\n` : ""}
`;
		}
		stepNum++;
	...
	if (classChanges.length)
		summaryParts.push(
			`${classChanges.length} class change${classChanges.length === 1 ? "" : "s"}`,
		);
	if (messages.length)
		summaryParts.push(
	...
	if (componentDrops.length)
		summaryParts.push(
			`${componentDrops.length} component drop${componentDrops.length === 1 ? "" : "s"}`,
		);
	if (textChanges.length)
		summaryParts.push(
			`${textChanges.length} text change${textChanges.length === 1 ? "" : "s"}`,
		);

	const resultsPart = classChanges
		.map((p) => `     { "patchId": "${p.id}", "success": true }`)
		.join(",\n");
	const designResultsPart = designs
		.map((p) => `     { "patchId": "${p.id}", "success": true }`)
		.join(",\n");
	const dropResultsPart = componentDrops
		.map((p) => `     { "patchId": "${p.id}", "success": true }`)
		.join(",\n");
	const textResultsPart = textChanges
		.map((p) => `     { "patchId": "${p.id}", "success": true }`)
		.join(",\n");
	const allResultsPart = [
		resultsPart,
		designResultsPart,
		dropResultsPart,
		textResultsPart,
	...
	if (classChanges.length || componentDrops.length || textChanges.length) {
		...
		if (textChanges.length) {
			step1 +=
				"\n   For text changes: find the text content in the source and replace it with the new text.";
		}
		if (messages.length) {
			step1 +=
...
${allResultsPart}
   ...
   \`\`\`
   (Only report results for class-change, design, component-drop, and text-change patches — messages are informational.)
...
export function registerMcpTools(mcp: McpServer, deps: McpToolDeps): void {
					...
					text: JSON.stringify(
						{
							isComplete: false,
							nextAction:
								"implement all patches in this commit, call mark_change_implemented, then call implement_next_change again",
	...
	mcp.tool(
			...
			"After calling this, you MUST call implement_next_change again to continue processing.",
		{
			commitId: z
				.string()
				.optional()
				.describe("The commit ID (new commit-based flow)"),
			results: z
				.array(
					z.object({
				...
				.optional()
				.describe(
					"Per-patch results (class-change patches only — skip message patches)",
				),
			ids: z
				.array(z.string())
				.optional()
				.describe("Legacy: Patch IDs to mark as implemented"),
		},
				...
				content: [
					{
						...
						text: JSON.stringify({
							...
							isComplete: false,
							nextAction:
								"call implement_next_change NOW to process the next change",
					...
					},
					{
						type: "text" as const,
						text:
							`✅ Marked ${moved} change(s) as implemented.\n\n` +
	...
	mcp.tool(
		...
		{
			status: z
				.enum(["staged", "committed", "implementing", "implemented", "error"])
				.optional()
		...
		async ({ status }) => {
			if (status) {
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify(getByStatus(status), null, 2),
						},
					],
				};
			}
			...
			return {
				content: [
					{ type: "text" as const, text: JSON.stringify(queueState, null, 2) },
				],
			};
		},
	);
