...
export async function resolveTailwindConfig(): Promise<TailwindThemeSubset> {
  ...
  if (configPath) {
    try {
      ...
      console.error(`[tailwind] Resolved config from ${configPath}`);
      return cached;
    } catch (err) {
      console.error("[tailwind] Failed to resolve config, using defaults:", err);
    }
  } else {
    console.error("[tailwind] No tailwind.config found — using defaults.");
  }
...
export async function generateCssForClasses(classes: string[]): Promise<string> {
  ...
  // @ts-expect-error — tailwindcss lacks NodeNext-compatible exports
  const tailwindPlugin = (await import("tailwindcss")).default;
