// Tailwind v4 theme resolver + CSS generator
// Uses the TARGET project's tailwindcss install via createRequire(cwd)
 ...
 * Compiles @import "tailwindcss" (the standard v4 entrypoint) and returns
 * the build() function that can generate CSS for any set of utility classes.
 */
...
export async function resolveTailwindConfig(): Promise<TailwindThemeSubset> {
  ...
  // transparent isn't a CSS var — add it manually
  if (!colors["transparent"]) colors["transparent"] = "transparent";

  // --- Spacing (v4 uses calc(var(--spacing) * N) — provide the standard scale keys) ---
  ...
  console.error("[tailwind] Resolved theme:", {
